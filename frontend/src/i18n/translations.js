export const translations = {
  pt: {
    // Header / Geral
    home: "Início",
    resume: "Currículo",
    projects: "Projetos",
    analytics: "Análises",
    contact: "Contato",
    experience: "Experiência",
    education: "Educação",
    skills: "Habilidades",

    // Hero
    homeTitle: "Analista de Dados",
    about:
      "Transformando dados em insights estratégicos através de análise, visualização e automação.",

    //Projetos
    projectsDescription:
      "Aqui estão alguns dos meus projetos recentes que demonstram minhas habilidades em análise de dados, visualização e automação.",
    searchProjects: "Buscar projetos...",

    // Filtros e Ordenação
    filterStack: "Tecnologias",
    selectStack: "Selecionar tecnologias",
    clearFilters: "Limpar",
    searchStack: "Buscar tecnologia...",

    sortLabel: "Ordenar",
    sortNewest: "Data (Mais recentes)",
    sortOldest: "Data (Mais antigos)",
    sortNameAsc: "Nome (A-Z)",
    sortNameDesc: "Nome (Z-A)",

    loadingProjects: "Carregando projetos...",

    notfind: "Nenhum projeto encontrado",

    // VSCode Viewer
    vscodeLoading: "Carregando projeto...",
    vscodeNoFile: "Selecione um arquivo para visualizar seu conteúdo.",

    // RESUME
    resumeRole:
      "Analista de Dados | SQL • Python • Power BI • ETL/ELT",
    resumeSummary:
      "Analista de Dados com mais de 4 anos de experiência desenvolvendo soluções analíticas que conectam estratégia de negócio a dados. Especialista em automação, integração e visualização, com foco em confiabilidade, governança e suporte à tomada de decisão.",

    resumeSkills: [
      "SQL (consultas avançadas, otimização e procedures)",
      "Python (ETL, automação, Pandas e NumPy)",
      "Power BI (Dashboards, DAX, Power Query, Dataflows)",
      "Modelagem de Dados e Performance",
      "ETL / ELT e Integração de Dados",
      "Data Governance e Qualidade de Dados",
      "Visualização e Data Storytelling",
      "Excel Avançado (Power Pivot, VBA)",
      "Git / GitHub",
      "Superset, Tableau e Looker Studio",
      "Automação de processos com n8n",
      "Fundamentos de Azure Data Services",
    ],

    resumeExperienceRole: "Analista de Dados",
    resumeExperiencePeriod: "Jul 2021 – Presente",

    resumeExperienceAchievements: [
      "Automação de 24 relatórios manuais em dashboards Power BI com atualização horária.",
      "Redução do tempo de processamento de relatórios financeiros críticos de 8h para 4h.",
      "Desenvolvimento de mais de 20 dashboards estratégicos para áreas como Financeiro, Vendas e Operações.",
      "Centralização de dados de múltiplos ERPs com rotinas de validação e governança.",
      "Criação e manutenção de pipelines ETL/ELT utilizando SQL e Python.",
      "Construção de modelos financeiros e projeções de fluxo de caixa para suporte a investimentos.",
      "Definição de KPIs e indicadores de rentabilidade para suporte à tomada de decisão executiva.",
    ],

    resumeEducation:
      "Bacharelado em Engenharia Civil (2023) - Universidade Federal de Goiás",

    downloadCV: "Baixar Currículo",

    // Analytics
    analyticsTitle: "Analytics",
    analyticsDescription: "Explore insights e visualizações de dados dos meus projetos, mostrando tendências, uso de tecnologias e distribuição geográfica.",
    loadingAnalytics: "Carregando analytics...",
    analyticsProjectsOverTime: "Projetos ao Longo do Tempo",
    analyticsStacksUsage: "Uso de Tecnologias",
    analyticsProjects: "Projetos",
    analyticsFiles: "Arquivos",
    analyticsStacks: "Tecnologias",
    analyticsCountries: "Países",
    noData: "Sem dados disponíveis",

    // Footer
    footerTitle: "Vamos nos conectar",
    footerCTA: "Fale comigo",
    footerDescription:
      "Analista de Dados focado em gerar valor através de dados, automação de processos e visualizações claras para tomada de decisão.",

    navigation: "Navegação",
    newsletter: "Receba novidades",
    emailPlaceholder: "Seu email",
    rights: "Todos os direitos reservados.",

    terms: "Termos de Uso",
    privacy: "Política de Privacidade",
  },

  en: {
    // Header / General
    home: "Home",
    resume: "Resume",
    projects: "Projects",
    analytics: "Analytics",
    contact: "Contact",
    experience: "Experience",
    education: "Education",
    skills: "Skills",

    // Hero
    homeTitle: "Data Analyst",
    about:
      "Transforming data into strategic insights through analysis, visualization and automation.",

    // Projects
    projectsDescription:
      "Here are some of my recent projects that showcase my skills in data analysis, visualization, and automation.",
    searchProjects: "Search projects...",

    // Filters & Sort
    filterStack: "Stacks",
    selectStack: "Select technologies",
    clearFilters: "Clear",
    searchStack: "Search stack...",

    sortLabel: "Sort by",
    sortNewest: "Date (Newest)",
    sortOldest: "Date (Oldest)",
    sortNameAsc: "Name (A-Z)",
    sortNameDesc: "Name (Z-A)",

    loadingProjects: "Loading projects...",

    notfind: "No projects found",

    // VSCode Viewer
    vscodeLoading: "Loading project...",
    vscodeNoFile: "Select a file to view its content.",

    // RESUME
    resumeRole:
      "Data Analyst | SQL • Python • Power BI • ETL/ELT",
    resumeSummary:
      "Data Analyst with over 4 years of experience developing analytical solutions that connect business strategy with data. Specialized in automation, integration, and visualization, focusing on reliability, governance, and decision support.",

    resumeSkills: [
      "SQL (advanced queries, optimization, stored procedures)",
      "Python (ETL, automation, Pandas and NumPy)",
      "Power BI (Dashboards, DAX, Power Query, Dataflows)",
      "Data Modeling and Performance",
      "ETL / ELT Pipelines",
      "Data Governance and Quality",
      "Visualization and Data Storytelling",
      "Advanced Excel (Power Pivot, VBA)",
      "Git / GitHub",
      "Superset, Tableau, and Looker Studio",
      "Process automation with n8n",
      "Azure Data Services fundamentals",
    ],

    resumeExperienceRole: "Data Analyst",
    resumeExperiencePeriod: "Jul 2021 – Present",

    resumeExperienceAchievements: [
      "Automated 24 manual reports into Power BI dashboards with hourly updates.",
      "Reduced processing time of critical financial reports from 8h to 4h.",
      "Developed 20+ strategic dashboards for Finance, Sales, and Operations.",
      "Centralized data from multiple ERPs with validation and governance routines.",
      "Designed and maintained ETL/ELT pipelines using SQL and Python.",
      "Built financial models and cash flow forecasts to support investments.",
      "Defined KPIs and profitability indicators for executive decision-making.",
    ],

    resumeEducation:
      "Bachelor’s Degree in Civil Engineering (2023) - Federal University of Goiás",

    downloadCV: "Download CV",

    // Analytics
    analyticsTitle: "Analytics",
    analyticsDescription: "Explore insights and data visualizations from my projects, showcasing trends, technology usage, and geographic distribution.",
    loadingAnalytics: "Loading analytics...",
    analyticsProjectsOverTime: "Projects Over Time",
    analyticsStacksUsage: "Technology Stack Usage",
    analyticsProjects: "Projects",
    analyticsFiles: "Files",
    analyticsStacks: "Stacks",
    analyticsCountries: "Countries",
    noData: "No data available",

    // Footer
    footerTitle: "Let’s connect",
    footerCTA: "Conctact me",
    footerDescription:
      "Data Analyst focused on creating value through data analysis, process automation and clear visualizations for decision-making.",

    navigation: "Navigation",
    newsletter: "Get the latest updates",
    emailPlaceholder: "Email address",
    rights: "All rights reserved.",

    terms: "Terms of Use",
    privacy: "Privacy Policy",
  },
};